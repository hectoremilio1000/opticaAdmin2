...
export default function CajaUpdateForm(props) {
  const {
    id: idProp,
    caja: cajaModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = cajaRecord
      ? { ...initialValues, ...cajaRecord }
      : initialValues;
    setFechaApertura(cleanValues.fechaApertura);
    setFechaCierre(cleanValues.fechaCierre);
    setMontoInicial(cleanValues.montoInicial);
    setMontoFinal(cleanValues.montoFinal);
    setEstado(cleanValues.estado);
    setErrors({});
  };
  const [cajaRecord, setCajaRecord] = React.useState(cajaModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Caja, idProp)
        : cajaModelProp;
      setCajaRecord(record);
    };
    queryData();
  }, [idProp, cajaModelProp]);
  React.useEffect(resetStateValues, [cajaRecord]);
  const validations = {
    fechaApertura: [],
    fechaCierre: [],
    montoInicial: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Caja.copyOf(cajaRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "CajaUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || cajaModelProp)}
